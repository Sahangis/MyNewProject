...
int main()
{
	...
	else if (answer == 'y')
	{
		...
		do {
			...
			cin >> ans;
			if (ans != 'v' or ans != 'm') cout << "ivesta netinkama reiksme, prasome ivesti vel " << endl;
		} while (ans != 'v' or ans != 'm');
		dat* a = new dat[n];
		for (int i = 0; i < n; i++) input(i, a[i], ans);
		rasymas(a, 10, ans);
		delete[] a;
	}
