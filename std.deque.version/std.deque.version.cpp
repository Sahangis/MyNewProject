...
int main()
{
	...
	string answer, ats;
	do {
		cout << "ar norite kruti rezultatu faila ? {0} ar norite generuoti 5 duomenu failus ? {1} (Duomenys saugomi Dequose)" << endl;
		cin >> ats;
		...
		else if (answer == "s")
		{
			...
			for (int i = 0; i < 5; i++)
			{
				...
				try
				{
					...
					std::cout << "Failo nuskaitymas tesiai i deqa uztruko: " << read_diff.count() << " s\n";
					fd.close();
					...
					std::cout << "Duomenu isrykiavimas ir isskyrimas i du atskirus kieteku, ir vargsiuku dequs uztruko: " << sort_diff.count() << " s\n";
				}
